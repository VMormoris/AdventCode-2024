...
pub fn main() !void {   
    const equations = try readFileAsSting("part2/input.txt");
    var sum: i64 = 0;
...
fn hasSolution(equation: Equation) !bool {
    ...
    for (numbers[1..]) |number| {
        ...
        for (res_clone.items) |result| {
            ...
            try results.append(result + number); 
            try results.append(try concat(result, number));
        }
    }
...
fn concat(lhs: i64, rhs: i64) !i64 {
    const lhs_str = try std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{lhs});
    defer std.heap.page_allocator.free(lhs_str);
    const rhs_str = try std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{rhs});
    defer std.heap.page_allocator.free(rhs_str);
    
    var result_str = std.ArrayList(u8).init(std.heap.page_allocator);
    defer result_str.deinit();
//
    try result_str.appendSlice(lhs_str);
    try result_str.appendSlice(rhs_str);

    const number = try std.fmt.parseInt(i64, result_str.items, 10);
    // std.log.debug("{d}", .{number});

    return number; //try std.fmt.parseInt(i64, result_str.items, 10);
}
