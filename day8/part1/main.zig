...
pub fn main() !void {
    const antenas_map = try readFileLineByLine("part1/input.txt");
    var antinode_map = try readFileLineByLine("part1/input.txt");
...
fn proccesFrequency(antenas_map: [][]const u8, antinode_map: [][]u8, frequency: u8) !void {
    ...
    defer {
        std.log.debug("For frequency: {c} found {d} posisitions", .{frequency, positions.items.len});
        positions.deinit();
    }
